...
`,c.appendChild(E)}),W.appendChild(c),n_();return}if(l==="/glitchmode"){if($=!$,$){w_.play();const i=["[terminal.exe v͠i͟r͘t̡ual̡ cor͢e r̕͢eb͘͟oot...]",">>> GlitchMODE INITIALIZED","ghost signal corrupted... attempting resync...","██▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒"],c=["#ff3399","#cc00ff","#ff3300","#00ffff"];for(let I=0;I<i.length;I++){const E=e_(i[I]);g(E,c[I%c.length]),await new Promise(u=>setTimeout(u,400))}}else g("🧼 Glitchmode deactivated","#ff3399");return}if(l==="/exit"){g("👻 The ghost vanishes into the datastream... refreshing...","#999"),setTimeout(()=>location.reload(),1500);return}if(l==="/ghostlog"){V_();const i=document.getElementById("ghostlog-download");i.style.display="inline-block",i.onclick=()=>{const I=JSON.parse(localStorage.getItem("ghostlog")||"[]").map(S=>`[${S.timestamp}]
> You: ${S.prompt}
> Ghost: ${S.response}
