...
@
@@
`;K.parseFont("Standard",v_);K.parseFont("Doom",O_);K.parseFont("Slant",W_);K.parseFont("Cyberlarge",H_);const W=document.getElementById("terminal-input"),H=document.getElementById("output"),P=document.getElementById("send-btn"),L_=document.getElementById("sfx-send"),M_=document.getElementById("sfx-glitch");function C(E,I="#00ffcc"){const s=document.createElement("span");s.style.color=I,s.textContent=E+`
`,H.appendChild(s),requestAnimationFrame(()=>{H.scrollTop=H.scrollHeight})}function I_(E){const I=["̷","͟","͜","̶","͡","͠","͢","̸","⸸","⚠","⛧"];return E.split("").map(s=>Math.random()<.1?I[Math.floor(Math.random()*I.length)]+s:Math.random()<.05?"":s).join("")}let s_,$=null;function e_(){const E=[".","..","..."],I=["#ff66cc","#ff3399"];let s=0;$=document.createElement("span"),$.style.color=I[0],H.appendChild($),s_=setInterval(()=>{$.textContent=`> 🧠 Summoning${E[s%E.length]}`,$.style.color=I[s%I.length],s++},400)}function U(){clearInterval(s_),$&&($.remove(),$=null)}function $_(E,I){const s=JSON.parse(localStorage.getItem("ghostlog")||"[]");s.push({timestamp:new Date().toISOString(),prompt:E,response:I}),localStorage.setItem("ghostlog",JSON.stringify(s))}function p_(){const E=JSON.parse(localStorage.getItem("ghostlog")||"[]");if(!E.length)return C("> No ghost memories stored.","#777");E.forEach((I,s)=>{C(`
${s+1}. [${I.timestamp}]`,"#ccc"),C(`> You: ${I.prompt}`,"#ff66cc"),C(`> Ghost: ${I.response}`,"#00ffff")})}function P_(E,I=80,s=50){return new Promise((A,l)=>{const u=document.getElementById("ascii-canvas"),d=u.getContext("2d"),c=new Image;c.crossOrigin="anonymous",c.src=E,c.onload=()=>{u.width=I,u.height=s,d.drawImage(c,0,0,I,s);const M=d.getImageData(0,0,I,s).data,D=[" ",".",":","-","=","+","*","#","%","@"];let F="";for(let b=0;b<s;b++){let G="";for(let V=0;V<I;V++){const x=(b*I+V)*4,Y=M[x],j=M[x+1],J=M[x+2],Q=(Y+j+J)/3,q=Math.floor(Q/255*(D.length-1));G+=D[q]}F+=G+`
...
${A}:
${l.name}
${l.card}`,"#ff66cc")}else if(E==="Celtic cross"){const A=["You","Your obstacle","Root of the matter","The past","What’s above you","The near future","Your attitude","Others involved","Hopes/Fears","Outcome"];for(let l of A){await s(500);const u=I();C(`
${l}:
${u.name}
${u.card}`,"#cc00ff")}}else if(E==="Show me the deck")for(let A of n_)await s(100),C(`${A.name}
...
`].forEach(s=>C(s,"#ff66cc"));return}if(E==="/glitchmode"){if(X=!X,X){M_.play();const I=["[terminal.exe v͠i͟r͘t̡ual̡ cor͢e r̕͢eb͘͟oot...]",">>> GlitchMODE INITIALIZED","ghost signal corrupted... attempting resync...","██▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒"],s=["#ff3399","#cc00ff","#ff3300","#00ffff"];for(let A=0;A<I.length;A++){const l=I_(I[A]);C(l,s[A%s.length]),await new Promise(u=>setTimeout(u,400))}}else C("🧼 Glitchmode deactivated","#ff3399");return}if(E==="/exit"){C("👻 The ghost vanishes into the datastream... refreshing...","#999"),setTimeout(()=>location.reload(),1500);return}if(E==="/ghostlog"){p_();const I=document.getElementById("ghostlog-download");I.style.display="inline-block",I.onclick=()=>{const A=JSON.parse(localStorage.getItem("ghostlog")||"[]").map(c=>`[${c.timestamp}]
> You: ${c.prompt}
> Ghost: ${c.response}

`).join(""),l=new Blob([A],{type:"text/plain"}),u=URL.createObjectURL(l),d=document.createElement("a");d.href=u,d.download="ghostlog.txt",d.click(),URL.revokeObjectURL(u)};return}if(E.startsWith("/textify ")){const s=`You're an aesthetic CLI stylist with an unhinged sense of creativity.
