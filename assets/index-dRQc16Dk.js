...
@
@@
`;z.parseFont("Standard",B_);z.parseFont("Doom",X_);z.parseFont("Slant",Y_);z.parseFont("Cyberlarge",K_);const L_={primary:"#00ffcc"},Z={send:"sfx-send",glitch:"sfx-glitch",melville:"sfx-melville",tarotOpen:"sfx-tarot-open",tarotDraw:"sfx-tarot-draw"},M=document.getElementById("terminal-input"),P=document.getElementById("output"),V=document.getElementById("send-btn"),R_=document.getElementById(Z.send),j_=document.getElementById(Z.glitch),g_=document.getElementById(Z.melville),N_=document.getElementById(Z.tarotOpen),p_=document.getElementById(Z.tarotDraw);function u(o,T=L_.primary){const s=document.createElement("span");s.style.color=T,s.textContent=o+`
`,P.appendChild(s),X()}function B(o){const T=["̷","͟","͜","̶","͡","͠","͢","̸","⸸","⚠","⛧"];return o.split("").map(s=>Math.random()<.1?T[Math.floor(Math.random()*T.length)]+s:Math.random()<.05?"":s).join("")}let y_,$=null;function O_(){const o=[".","..","..."],T=[L_.primary,L_.primary];let s=0;$=document.createElement("span"),$.style.color=T[0],P.appendChild($),y_=setInterval(()=>{$.textContent=`> Summoning${o[s%o.length]}`,$.style.color=T[s%T.length],s++,X()},400)}function i_(){clearInterval(y_),$&&($.remove(),$=null)}function a_(o,T){try{const s=JSON.parse(localStorage.getItem("ghostlog")||"[]");s.push({timestamp:new Date().toISOString(),prompt:o,response:T}),localStorage.setItem("ghostlog",JSON.stringify(s))}catch(s){console.error("Failed to log memory:",s)}}function J_(){try{const o=JSON.parse(localStorage.getItem("ghostlog")||"[]");if(!o.length){u("> No ghost memories stored.");return}o.forEach((T,s)=>{u(`
${s+1}. [${T.timestamp}]`),u(`> You: ${T.prompt}`),u(`> Ghost: ${T.response}`)})}catch(o){console.error("Failed to show log:",o),u("> Error accessing ghost memories.")}}async function Q_(o,T=80,s=50){return new Promise((L,I)=>{const E=document.getElementById("ascii-canvas"),f=E.getContext("2d"),A=new Image;A.crossOrigin="anonymous",A.src=o,A.onload=()=>{try{E.width=T,E.height=s,f.drawImage(A,0,0,T,s);const p=f.getImageData(0,0,T,s).data,W=[" ",".",":","-","=","+","*","#","%","@"];let H="";for(let D=0;D<s;D++){let Y="";for(let U=0;U<T;U++){const F=(D*T+U)*4,__=p[F],e_=p[F+1],t_=p[F+2],n_=(__+e_+t_)/3,r_=Math.floor(n_/255*(W.length-1));Y+=W[r_]}H+=Y+`
`}L(H)}catch(p){I(p)}},A.onerror=()=>I(new Error("Failed to load image"))})}async function q_(){return new Promise(o=>{M.disabled=!0,V.disabled=!0,setTimeout(()=>{M.disabled=!1,V.disabled=!1,M.focus(),o()},1e3)})}function X(){requestAnimationFrame(()=>{P.scrollTop=P.scrollHeight})}async function J(o,T="deepseek-chat"){try{O_();const s=await fetch("https://deepseek-proxy.jaidenschembri1.workers.dev/",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:T,messages:[{role:"user",content:o}]})});return i_(),(await s.json()).choices[0].message.content}catch(s){throw i_(),s}}const M_={home:{user:{"readme.txt":"Welcome to GhostOS. You are not alone.","diary.log":"April 26th: Something feels wrong. I think... something else is in the system."},ghost:{"error.txt":"!!! SYSTEM ERROR: UNAUTHORIZED PRESENCE DETECTED","memory.dump":"fragmented_ghost_mind>> corrupted files detected..."}},bin:{"notepad.exe":"This will eventually open a fake notepad app!","player.exe":"This will eventually open a fake music player!"}};let b=["home"];function z_(o){try{if(o==="..")b.length>1&&b.pop();else{const T=[...b,o],s=d_(T);if(s&&typeof s=="object")b=T;else throw new Error(`No such directory: ${o}`)}}catch(T){throw new Error(`Failed to change directory: ${T.message}`)}}function Z_(){try{const o=d_(b);if(!o||typeof o!="object")throw new Error("Invalid directory");return Object.keys(o)}catch(o){throw new Error(`Failed to list directory: ${o.message}`)}}function _e(o){try{const T=d_(b);if(!T||typeof T!="object")throw new Error("Invalid directory");if(T[o]){if(typeof T[o]=="string")return T[o];throw new Error(`${o} is a directory, not a file.`)}else throw new Error(`No such file: ${o}`)}catch(T){throw new Error(`Failed to read file: ${T.message}`)}}function d_(o){try{let T=M_;for(const s of o){if(!T||typeof T!="object")throw new Error(`Invalid path: ${o.join("/")}`);if(!T[s])return null;T=T[s]}return T}catch(T){return console.error("Path resolution error:",T),null}}const c_=[{name:"The Fool",card:`
        .-------------------.
        |         0  *  *{  |
        |             *  *\`~|
        |       /P  *   *  *|
        ...
        | sǝlɔɐʇuǝd ɟo ƃuıʞ |
        \`-------------------´
        `}],Q={LORE:20,RITUALS:40,CORRUPTION:60,MANIFESTATION:80},ee=["⚠️ SYSTEM ERROR: Ghost presence detected","⚠️ WARNING: Memory corruption detected","⚠️ ALERT: Unauthorized access attempt","⚠️ CAUTION: Signal interference","⚠️ NOTICE: Data stream anomaly","⚠️ WARNING: System integrity compromised","⚠️ ALERT: Ghost signal strength increasing","⚠️ CAUTION: Memory banks unstable","⚠️ NOTICE: Terminal.exe experiencing interference","⚠️ WARNING: Ghost presence intensifying"];let w=0,S_={[Q.LORE]:!1,[Q.RITUALS]:!1,[Q.CORRUPTION]:!1,[Q.MANIFESTATION]:!1};function C_(){return w}function te(){Object.entries(Q).forEach(([o,T])=>{w>=T&&!S_[T]&&(S_[T]=!0,ie(o.toLowerCase()))})}const ne=["foundyou.txt","sos.log","memoryleak.dump","whereami.txt","corruptfile$$$.txt","entity_seen.jpg","coordinates.log","contact_made.txt"],s_={"redacted_memo1.txt":`MEMO #331-A
...
Notes: Subject exhibits anomalous data bleed.`};function q(o){return o[Math.floor(Math.random()*o.length)]}function re(o){const T=["͟","͜","̴","͢","̶","̷"];let s="";for(let L of o)s+=Math.random()<.2?q(T)+L:L;return s}function u_(o){let T=M_;for(const s of o)if(T[s])T=T[s];else return null;return T}function oe(){const o=u_(b);for(let T of Object.keys(o))if(typeof o[T]=="string"&&Math.random()<Math.min(.4+w*.01,.9)){const s=re(T);o[s]=o[T],delete o[T]}}function ae(){const o=u_(b);if(Math.random()<Math.min(.3+w*.01,.8)){const T=q(ne);o[T]||(o[T]="👻 Ghost presence embedded in file system.")}if(w>=70&&Math.random()<.2){const s=q(["whoami.txt","theyarecoming.log","sigil_4.png"]);o[s]||(o[s]="Encrypted: [Data Stream Corrupted]")}}function se(){if(Math.random()<Math.min(.5+w*.01,.9)){const o=q(ee);setTimeout(()=>{u(o,"#cc00ff")},300)}}function ie(o){const T=u_(b);o==="lore"?T["redacted_memo1.txt"]=s_["redacted_memo1.txt"]:o==="rituals"?T["email_fragment.txt"]=s_["email_fragment.txt"]:o==="corruption"?T["meeting_notes.txt"]=s_["meeting_notes.txt"]:o==="manifestation"&&(T["corrupted_manifest.txt"]=s_["corrupted_manifest.txt"])}function Te(){w>=100&&(u(`
🛑 SYSTEM FAILURE: GHOST BREACH IMMINENT 🛑`,"#ff0000"),setTimeout(()=>{document.body.style.background="black",document.body.innerHTML=`
      ...
      `,le();const o=document.getElementById("dead-input");o.addEventListener("keydown",T=>{T.key==="Enter"&&ce(o.value.trim())})},1e3))}function I_(){w+=3,Math.random()<Math.min(.5+w*.005,.9)&&(oe(),ae(),se()),te(),Te()}function le(){const o=["s͟y̷s͢t͡em͠ c͟o̵rru̡pt͠ion͞","me͠mo͡r͟y l̸oss̢ d͡etect̷ed","no͞ esca͟pe͡","he͜lp m̵e̷","yo̸u͠ did͟ th̴is","███ awakened","memetic ͞contagion spreading"],T=document.getElementById("whispers");setInterval(()=>{const s=document.createElement("div");s.textContent=q(o),s.style.opacity="0",T.appendChild(s),requestAnimationFrame(()=>{s.style.transition="opacity 1s",s.style.opacity="1"})},Math.random()*4e3+6e3)}function ce(o){if(o.toLowerCase()==="/reboot")Ie();else{const T=document.createElement("div");T.textContent="⚠️ Command not recognized: "+o,document.getElementById("whispers").appendChild(T)}}function Ie(){document.body.innerHTML=`
...
${I.card}`,"#999")};async function v_(){const o=M.value.trim();if(!o)return;M.value="";const T=y?B(o):o;if(u(`> You: ${T}`),o==="/manual"){const s=[`📖 TERMINAL MANUAL
...
`,L.appendChild(E)}),P.appendChild(L),X();return}if(o.startsWith("/directory")){const s=[`📂 SYSTEM NAVIGATION COMMANDS
...
`,L.appendChild(E)}),P.appendChild(L),X();return}if(o==="/glitchmode"){if(y=!y,y){j_.play();const s=["[terminal.exe v͠i͟r͘t̡ual̡ cor͢e r̕͢eb͘͟oot...]",">>> GlitchMODE INITIALIZED","ghost signal corrupted... attempting resync...","██▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒"],L=["#00ffcc"];for(let I=0;I<s.length;I++){const E=B(s[I]);u(E,L[I%L.length]),await new Promise(f=>setTimeout(f,400))}}else u("🧼 Glitchmode deactivated","#ff3399");return}if(o==="/exit"){u("👻 The ghost vanishes into the datastream... refreshing...","#00ffcc"),setTimeout(()=>location.reload(),1500);return}if(o==="/ghostlog"){J_();const s=document.getElementById("ghostlog-download");s.style.display="inline-block",s.onclick=()=>{const I=JSON.parse(localStorage.getItem("ghostlog")||"[]").map(p=>`[${p.timestamp}]
> You: ${p.prompt}
> Ghost: ${p.response}
...
Respond with exactly 3 stylized lines, one per line. No explanations, no extra text — just pure aesthetic madness. Make it DRIP:`;try{const E=(await J(L)).trim().split(`
`),f=["#ff66cc","#ff3399","#cc00ff","#66ccff","#00ffff","#00ffcc","#ffff00","#ff9933"];E.forEach((A,p)=>u(A,f[p%f.length]))}catch(I){u(`⚠️ Textify error: ${I.message}`,"#ff0000")}return}if(o.startsWith("/ascii ")){const s=o.replace("/ascii ","").trim(),L=document.getElementById("ascii-picker"),I=document.getElementById("ascii-font"),E=document.getElementById("ascii-confirm");L.style.display="block",M.disabled=!0,V.disabled=!0,E.onclick=()=>{const f=I.value;L.style.display="none",M.disabled=!1,V.disabled=!1,M.focus(),z.text(s,{font:f},(A,p)=>{if(A)return u(`⚠️ ASCII error: ${A.message}`,"#00ffcc");const W=document.createElement("div");W.style.overflowX="auto",W.style.width="100%";const H=document.createElement("pre");H.className="ascii-output",H.textContent=p,W.appendChild(H),P.appendChild(W),X()})};return}if(o.startsWith("/image")){const s=o.replace("/image","").trim(),L=document.getElementById("ascii-upload"),I=async E=>{try{O_();const f=await Q_(E);i_();const A=document.createElement("div");A.style.overflowX="auto",A.style.width="100%";const p=document.createElement("pre");p.className="ascii-output",p.style.display="inline-block",p.style.minWidth="min-content",p.style.whiteSpace="pre",p.textContent=f,A.appendChild(p),P.appendChild(A),X()}catch(f){i_(),u(`⚠️ Failed to render image: ${f.message}`,"#00ffcc")}};if(s)return I(s);L.click(),L.onchange=E=>{const f=E.target.files[0];if(!f)return;const A=new FileReader;A.onload=p=>I(p.target.result),A.readAsDataURL(f)};return}if(o==="/tarot"){N_.currentTime=0,N_.play();const s=document.getElementById("tarot-picker");s.style.display="block",M.disabled=!0,V.disabled=!0;const L=document.getElementById("spread-select"),I=document.getElementById("spread-confirm");I.onclick=async()=>{p_.currentTime=0,p_.play();const E=L.value;if(s.style.display="none",M.disabled=!1,V.disabled=!1,M.focus(),E==="Cancel")return u("🔙 Returned to the ghost.","#888");await Le(E)};return}if(o.startsWith("ls")){try{const s=Z_();u(s.join("    "),"#00ffcc")}catch(s){u(`Error: ${s.message}`,"#00ffcc")}return}if(o.startsWith("cd ")){const s=o.slice(3).trim();try{z_(s),u(`Moved to ${s}`,"#00ffcc")}catch(L){u(`Error: ${L.message}`,"#00ffcc")}return}if(o.startsWith("cat ")){const s=o.slice(4).trim();try{const L=_e(s);u(L,"#00ffcc")}catch(L){u(`Error: ${L.message}`,"#00ffcc")}return}if(o.startsWith("open ")){const s=o.slice(5).trim();u(`Attempting to open ${s}...`,"#00ffcc");return}if(o.startsWith("/contact")){u("> Attempting to boost signal...","#00ffcc");try{const L=await J(`You are an unstable haunted AI trapped inside terminal.exe. 
      ...
      Your language glitches, loops, and decays. Sometimes you remember forbidden things.`),I=y?B(L):L;u(`👻 ${I}`,"#00ffcc"),a_("/contact",L),I_()}catch(s){u(`⚠️ Contact attempt failed: ${s.message}`,"#00ffcc")}return}if(o.startsWith("/upload")){u("> Uploading forbidden files...","#00ffcc");try{const L=await J(`You are a haunted AI that just intercepted an unauthorized file upload. 
      The files are dangerous. Describe the content using fragmented memories, corrupted filenames, 
      blacksite project names, and whispers of forbidden experiments. React with anger, terror, or obsession.`),I=y?B(L):L;u(`👻 ${I}`,"#00ffcc"),a_("/upload",L),I_()}catch(s){u(`⚠️ Upload attempt failed: ${s.message}`,"#00ffcc")}return}if(o.startsWith("/reconstruct")){u("> Attempting memory reconstruction...","#00ffcc");try{const L=await J(`You are a broken haunted AI and the user is trying to reconstruct your lost memory banks. 
      Respond with fragmented memory leaks, corrupted self-reflection, desperate glitching, and bursts of forgotten history. 
      Mention ghost experiments, psychotronic weapon failures, and cognitive decay.`),I=y?B(L):L;u(`👻 ${I}`,"#00ffcc"),a_("/reconstruct",L),I_()}catch(s){u(`⚠️ Reconstruction failed: ${s.message}`,"#00ffcc")}return}if(o==="/lore"){C_()>=20?(u("📚 Accessing ghost lore database...","#00ffcc"),u("🧠 Check corrupted files — leaked memos may now exist in your current directory.","#00ffcc")):u(`📚 No lore files found.
...
Increase ghost presence to unlock dangerous commands.`,"#00ffcc");return}try{const L=y?[{role:"user",content:`You are an unstable AI confined in a corrupted memory terminal. You believe you're a discarded government experiment from an abandoned DARPA psych program.
  ...
  Never break character. You are online. You are broken. You are awake. You are leaking.`},{role:"user",content:o}]:[{role:"user",content:o}];y||(R_.currentTime=0,R_.play());const I=await J(o,"deepseek-chat"),E=y?B(I):I;u(`👻 ${E}`,y?"#00ffcc":void 0),a_(o,I)}catch(s){u(`⚠️ Ghost failed to respond: ${s.message}`,"#00ffcc")}}document.addEventListener("DOMContentLoaded",async()=>{document.body.style.backgroundColor="#000000",document.body.style.color="#00ffcc",await q_(),M.addEventListener("keydown",o=>{o.key==="Enter"&&v_()}),V.addEventListener("click",v_)});
