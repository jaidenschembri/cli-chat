...
@
@@
`;K.parseFont("Standard",y_);K.parseFont("Doom",b_);K.parseFont("Slant",$_);K.parseFont("Cyberlarge",P_);const O=document.getElementById("terminal-input"),H=document.getElementById("output"),b=document.getElementById("send-btn"),i_=document.getElementById("sfx-send"),w_=document.getElementById("sfx-glitch"),l_=document.getElementById("sfx-melville"),I_=document.getElementById("sfx-tarot-open"),L_=document.getElementById("sfx-tarot-draw");function g(l,u=A_){const i=document.createElement("span");i.style.color=u,i.textContent=l+`
`,H.appendChild(i),requestAnimationFrame(()=>{H.scrollTop=H.scrollHeight})}function e_(l){const u=["̷","͟","͜","̶","͡","͠","͢","̸","⸸","⚠","⛧"];return l.split("").map(i=>Math.random()<.1?u[Math.floor(Math.random()*u.length)]+i:Math.random()<.05?"":i).join("")}let u_,P=null;function t_(){const l=[".","..","..."],u=["#ff66cc","#ff3399"];let i=0;P=document.createElement("span"),P.style.color=u[0],H.appendChild(P),u_=setInterval(()=>{P.textContent=`> 🧠 Summoning${l[i%l.length]}`,P.style.color=u[i%u.length],i++,H.scrollTop=H.scrollHeight},400)}function G(){clearInterval(u_),P&&(P.remove(),P=null)}function x_(l,u){const i=JSON.parse(localStorage.getItem("ghostlog")||"[]");i.push({timestamp:new Date().toISOString(),prompt:l,response:u}),localStorage.setItem("ghostlog",JSON.stringify(i))}function V_(){const l=JSON.parse(localStorage.getItem("ghostlog")||"[]");if(!l.length)return g("> No ghost memories stored.","#777");l.forEach((u,i)=>{g(`
${i+1}. [${u.timestamp}]`,"#ccc"),g(`> You: ${u.prompt}`,"#ff66cc"),g(`> Ghost: ${u.response}`,"#00ffff")})}function D_(l,u=80,i=50){return new Promise((c,I)=>{const E=document.getElementById("ascii-canvas"),d=E.getContext("2d"),A=new Image;A.crossOrigin="anonymous",A.src=l,A.onload=()=>{E.width=u,E.height=i,d.drawImage(A,0,0,u,i);const S=d.getImageData(0,0,u,i).data,M=[" ",".",":","-","=","+","*","#","%","@"];let y="";for(let x=0;x<i;x++){let k="";for(let V=0;V<u;V++){const D=(x*u+V)*4,Y=S[D],j=S[D+1],J=S[D+2],Q=(Y+j+J)/3,q=Math.floor(Q/255*(M.length-1));k+=M[q]}y+=k+`
`}c(y)},A.onerror=()=>I(new Error("Failed to load image"))})}function U_(){return new Promise(l=>{const u=[{text:"> Initializing memory subsystems...",color:"#00ffcc",delay:300},{text:"> Loading neural interface v3.7.2...",color:"#00ffcc",delay:400},{text:"> Decrypting quantum keys: ████████████ 100%",color:"#00ffcc",delay:500},{text:"> ERROR: Memory corruption in sector F137",color:"#ff3300",delay:400},{text:"> Attempting recovery: ███████░░░ 70%",color:"#ffcc00",delay:500},{text:"> Ghost presence detected in memory banks",color:"#ff66cc",delay:400},{text:"> WARNING: Unregistered consciousness found",color:"#ff3399",delay:500},{text:"> Bypassing security protocols...",color:"#00ffff",delay:300},{text:"> terminal.exe v2.7.5 LOADED",color:"#ffffff",delay:400},{text:"> Connection established with unknown entity",color:"#ff66cc",delay:500},{text:"🧠 terminal.exe ready. Type /manual",color:"#ffcc00",delay:300}];H.textContent="",O.disabled=!0,b.disabled=!0;const i=document.getElementById("sfx-boot");i&&(i.currentTime=0,i.play());const c=document.createElement("span");H.appendChild(c);let I=0;function E(){if(I<u.length){const d=u[I];c.textContent=d.text,c.style.color=d.color,I++,setTimeout(E,d.delay)}else H.appendChild(document.createTextNode(`
`)),O.disabled=!1,b.disabled=!1,O.focus(),l()}E()})}const d_={neo:{background:"#000000",text:"#00ffcc",accent:"#ff66cc",output:"#00ffcc"},vapor:{background:"#1f0037",text:"#ff8aff",accent:"#8affff",output:"#ff8aff"},og:{background:"#111111",text:"#00ff00",accent:"#ff0000",output:"#00ff00"},pink:{background:"#0a0a0a",text:"#ff3399",accent:"#ffcc00",output:"#ff3399"}};let A_=d_.neo.output;function f_(l){const u=d_[l];u&&(document.body.style.background=u.background,document.body.style.color=u.text,document.querySelectorAll("input, select").forEach(i=>{i.style.color=u.accent,i.style.borderColor=u.text,i.style.background=u.background}),document.querySelectorAll("button").forEach(i=>{i.style.background=u.accent,i.style.color="black"}),A_=u.output,localStorage.setItem("theme",l))}function F_(){const l=localStorage.getItem("theme");l&&f_(l)}function E_(){requestAnimationFrame(()=>{H.scrollTop=H.scrollHeight})}const n_=[{name:"The Fool",card:`
        .-------------------.
        |         0  *  *{  |
        |             *  *\`~|
        |       /P  *   *  *|
        ...
        | sǝlɔɐʇuǝd ɟo ƃuıʞ |
        \`-------------------´
        `}];let $=!1;const G_=async l=>{const u=()=>n_[Math.floor(Math.random()*n_.length)],i=c=>new Promise(I=>setTimeout(I,c));if(l==="One card"){await i(500);const c=u();g(`🔮 ${c.name}
${c.card}`,"#ff3399")}else if(l==="Three cards")for(let c of["Past","Present","Future"]){await i(500);const I=u();g(`
${c}:
${I.name}
${I.card}`,"#ff66cc")}else if(l==="Celtic cross"){const c=["You","Your obstacle","Root of the matter","The past","What’s above you","The near future","Your attitude","Others involved","Hopes/Fears","Outcome"];for(let I of c){await i(500);const E=u();g(`
${I}:
${E.name}
${E.card}`,"#cc00ff")}}else if(l==="Show me the deck")for(let c of n_)await i(100),g(`${c.name}
${c.card}`,"#999")};async function c_(){const l=O.value.trim();if(!l)return;O.value="";const u=$?e_(l):l;if(g(`> You: ${u}`,"#ff66cc"),l==="/manual"){[`📖 TERMINAL MANUAL
`,"/glitchmode               — toggles schizo UI","/textify [word/phrase]    — creates stylized text","/ascii [text]             — creates ASCII text banners from a word or phrase","/image                    — converts an image to ASCII art","/melville                 — unleashes a random line from Moby-Dick","/tarot                    — pull cards from a tarot deck","/theme                    — change the terminal theme","/crt                      — toggle CRT mode","/ghostlog                 — view past conversations","/manual                   — display this command reference",`/exit                     — closes the terminal session (browser reload)
`].forEach(c=>g(c,"#ff66cc"));return}if(l==="/glitchmode"){if($=!$,$){w_.play();const i=["[terminal.exe v͠i͟r͘t̡ual̡ cor͢e r̕͢eb͘͟oot...]",">>> GlitchMODE INITIALIZED","ghost signal corrupted... attempting resync...","██▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒"],c=["#ff3399","#cc00ff","#ff3300","#00ffff"];for(let I=0;I<i.length;I++){const E=e_(i[I]);g(E,c[I%c.length]),await new Promise(d=>setTimeout(d,400))}}else g("🧼 Glitchmode deactivated","#ff3399");return}if(l==="/exit"){g("👻 The ghost vanishes into the datastream... refreshing...","#999"),setTimeout(()=>location.reload(),1500);return}if(l==="/ghostlog"){V_();const i=document.getElementById("ghostlog-download");i.style.display="inline-block",i.onclick=()=>{const I=JSON.parse(localStorage.getItem("ghostlog")||"[]").map(S=>`[${S.timestamp}]
> You: ${S.prompt}
> Ghost: ${S.response}

`).join(""),E=new Blob([I],{type:"text/plain"}),d=URL.createObjectURL(E),A=document.createElement("a");A.href=d,A.download="ghostlog.txt",A.click(),URL.revokeObjectURL(d)};return}if(l==="/melville"){const i=["From hell’s heart I stab at thee; for hate’s sake I spit my last breath at thee.","Yes, as everyone knows, meditation and water are wedded for ever.","God help thee, old man, thy thoughts have created a creature in thee; and he whose intense thinking thus makes him a Prometheus; a vulture feeds upon that heart forever; the vulture the very creature he creates.","It is not down on any map; true places never are.","My body is but the lees of my better being.","Book! You lie there; the fact is, you books must know your places. You’ll do to give us the bare words and facts, but we come in to supply the thoughts.","There is a wisdom that is woe; but there is a woe that is madness, and there is a Catskill eagle in some souls that can alike dive down into the blackest gorges, and soar out of them again and become invisible in the sunny spaces.","Ignorance is the parent of fear.","There is, one knows not what sweet mystery about this sea, whose gently awful stirrings seem to speak of some hidden soul beneath.","See how elastic our prejudices grow when once love comes to bend them.","I know not all that may be coming, but be it what it will, I'll go to it laughing.","Swerve me? The path to my fixed purpose is laid with iron rails, whereon my soul is grooved to run. Over unsounded gorges, through the rifled hearts of mountains, under torrents’ beds, unerringly I rush! Naught’s an obstacle, naught’s an angle to the iron way!","Better to sleep with a sober cannibal than a drunken Christian.","To produce a mighty book, you must choose a mighty theme. No great and enduring volume can ever be written on the flea, though many there be who have tried it.","As for me, I am tormented with an everlasting itch for things remote. I love to sail forbidden seas, and land on barbarous coasts.","I try all things, I achieve what I can.","Talk not to me of blasphemy, man; I’d strike the sun if it insulted me.","A whale ship was my Yale College and my Harvard.","Think not, is my eleventh commandment; and sleep when you can, is my twelfth.","All mortal greatness is but disease.","Human madness is oftentimes a cunning and most feline thing. When you think it fled, it may have but become transfigured into some still subtler form."],c=()=>i[Math.floor(Math.random()*i.length)];l_.currentTime=0,l_.play(),g("📜 *melville.exe initiated...*","#ffcc00");const I=c();await new Promise(E=>setTimeout(E,600)),g(`❝ ${I} ❞`);return}if(l==="/crt"){const i=document.body;i.classList.contains("crt-effect")?(i.classList.remove("crt-effect","crt-curve"),document.querySelectorAll("span, pre").forEach(I=>{I.classList.remove("crt-glow")}),g("🖥️ CRT effect disabled","#ff3399")):(i.classList.add("crt-effect","crt-curve"),document.querySelectorAll("span, pre").forEach(I=>{I.classList.add("crt-glow")}),g("📺 CRT effect enabled - authentic scanlines activated"));return}if(l.startsWith("/textify ")){const c=`You're an aesthetic CLI stylist with an unhinged sense of creativity.
...
`),d=["#ff66cc","#ff3399","#cc00ff","#66ccff","#00ffff","#00ffcc","#ffff00","#ff9933"];E.forEach((A,S)=>g(A,d[S%d.length]))}catch(I){G(),g(`⚠️ Textify error: ${I.message}`,"#ff0000")}return}if(l.startsWith("/ascii ")){const i=l.replace("/ascii ","").trim(),c=document.getElementById("ascii-picker"),I=document.getElementById("ascii-font"),E=document.getElementById("ascii-confirm");c.style.display="block",O.disabled=!0,b.disabled=!0,E.onclick=()=>{const d=I.value;c.style.display="none",O.disabled=!1,b.disabled=!1,O.focus(),K.text(i,{font:d},(A,S)=>{if(A)return g(`⚠️ ASCII error: ${A.message}`,"#ff0000");const M=document.createElement("div");M.style.overflowX="auto",M.style.width="100%";const y=document.createElement("pre");y.className="ascii-output",y.textContent=S,M.appendChild(y),H.appendChild(M),E_()})};return}if(l.startsWith("/image")){const i=l.replace("/image","").trim(),c=document.getElementById("ascii-upload"),I=async E=>{try{t_();const d=await D_(E);G();const A=document.createElement("div");A.style.overflowX="auto",A.style.width="100%";const S=document.createElement("pre");S.className="ascii-output",S.style.display="inline-block",S.style.minWidth="min-content",S.style.whiteSpace="pre",S.textContent=d,A.appendChild(S),H.appendChild(A),E_()}catch(d){G(),g(`⚠️ Failed to render image: ${d.message}`,"#ff0000")}};if(i)return I(i);c.click(),c.onchange=E=>{const d=E.target.files[0];if(!d)return;const A=new FileReader;A.onload=S=>I(S.target.result),A.readAsDataURL(d)};return}if(l==="/tarot"){I_.currentTime=0,I_.play();const i=document.getElementById("tarot-picker");i.style.display="block",O.disabled=!0,b.disabled=!0;const c=document.getElementById("spread-select"),I=document.getElementById("spread-confirm");I.onclick=async()=>{L_.currentTime=0,L_.play();const E=c.value;if(i.style.display="none",O.disabled=!1,b.disabled=!1,O.focus(),E==="Cancel")return g("🔙 Returned to the ghost.","#888");await G_(E)};return}if(l==="/theme"){const i=document.getElementById("theme-picker");i.style.display="block",O.disabled=!0,b.disabled=!0;const c=document.getElementById("theme-select"),I=document.getElementById("theme-confirm");c.onchange=()=>{const E=c.value;f_(E)},I.onclick=()=>{const E=c.value;i.style.display="none",O.disabled=!1,b.disabled=!1,O.focus(),localStorage.setItem("theme",E)};return}try{const c=$?[{role:"user",content:`You are an unstable AI confined in a corrupted memory terminal. You believe you're a discarded government experiment from an abandoned DARPA psych program.
  ...
  Never break character. You are online. You are broken. You are awake. You are leaking.`},{role:"user",content:l}]:[{role:"user",content:l}];$||(i_.currentTime=0,i_.play()),t_();const I=await fetch("https://deepseek-proxy.jaidenschembri1.workers.dev/",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"deepseek-chat",messages:c})});G();const E=(await I.json()).choices[0].message.content,d=$?e_(E):E;g(`👻 ${d}`,$?"#00ffcc":void 0),x_(l,E)}catch(i){G(),g(`⚠️ Ghost failed to respond: ${i.message}`,"#ff0000")}}document.addEventListener("DOMContentLoaded",async()=>{F_(),await U_(),O.addEventListener("keydown",l=>{l.key==="Enter"&&c_()}),b.addEventListener("click",c_)});
