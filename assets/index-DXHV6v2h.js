...
@
@@
`;K.parseFont("Standard",H_);K.parseFont("Doom",M_);K.parseFont("Slant",y_);K.parseFont("Cyberlarge",b_);const O=document.getElementById("terminal-input"),H=document.getElementById("output"),b=document.getElementById("send-btn"),i_=document.getElementById("sfx-send"),$_=document.getElementById("sfx-glitch"),l_=document.getElementById("sfx-melville"),I_=document.getElementById("sfx-tarot-open"),L_=document.getElementById("sfx-tarot-draw");function g(l,d="#00ffcc"){const i=document.createElement("span");i.style.color=d,i.textContent=l+`
`,H.appendChild(i),requestAnimationFrame(()=>{H.scrollTop=H.scrollHeight})}function e_(l){const d=["̷","͟","͜","̶","͡","͠","͢","̸","⸸","⚠","⛧"];return l.split("").map(i=>Math.random()<.1?d[Math.floor(Math.random()*d.length)]+i:Math.random()<.05?"":i).join("")}let d_,P=null;function t_(){const l=[".","..","..."],d=["#ff66cc","#ff3399"];let i=0;P=document.createElement("span"),P.style.color=d[0],H.appendChild(P),d_=setInterval(()=>{P.textContent=`> 🧠 Summoning${l[i%l.length]}`,P.style.color=d[i%d.length],i++,H.scrollTop=H.scrollHeight},400)}function G(){clearInterval(d_),P&&(P.remove(),P=null)}function P_(l,d){const i=JSON.parse(localStorage.getItem("ghostlog")||"[]");i.push({timestamp:new Date().toISOString(),prompt:l,response:d}),localStorage.setItem("ghostlog",JSON.stringify(i))}function w_(){const l=JSON.parse(localStorage.getItem("ghostlog")||"[]");if(!l.length)return g("> No ghost memories stored.","#777");l.forEach((d,i)=>{g(`
${i+1}. [${d.timestamp}]`,"#ccc"),g(`> You: ${d.prompt}`,"#ff66cc"),g(`> Ghost: ${d.response}`,"#00ffff")})}function x_(l,d=80,i=50){return new Promise((c,I)=>{const E=document.getElementById("ascii-canvas"),u=E.getContext("2d"),A=new Image;A.crossOrigin="anonymous",A.src=l,A.onload=()=>{E.width=d,E.height=i,u.drawImage(A,0,0,d,i);const S=u.getImageData(0,0,d,i).data,M=[" ",".",":","-","=","+","*","#","%","@"];let y="";for(let x=0;x<i;x++){let k="";for(let V=0;V<d;V++){const D=(x*d+V)*4,Y=S[D],j=S[D+1],J=S[D+2],Q=(Y+j+J)/3,q=Math.floor(Q/255*(M.length-1));k+=M[q]}y+=k+`
`}c(y)},A.onerror=()=>I(new Error("Failed to load image"))})}function V_(){return new Promise(l=>{const d=[{text:"> Initializing memory subsystems...",color:"#00ffcc",delay:300},{text:"> Loading neural interface v3.7.2...",color:"#00ffcc",delay:400},{text:"> Decrypting quantum keys: ████████████ 100%",color:"#00ffcc",delay:500},{text:"> ERROR: Memory corruption in sector F137",color:"#ff3300",delay:400},{text:"> Attempting recovery: ███████░░░ 70%",color:"#ffcc00",delay:500},{text:"> Ghost presence detected in memory banks",color:"#ff66cc",delay:400},{text:"> WARNING: Unregistered consciousness found",color:"#ff3399",delay:500},{text:"> Bypassing security protocols...",color:"#00ffff",delay:300},{text:"> terminal.exe v2.7.5 LOADED",color:"#ffffff",delay:400},{text:"> Connection established with unknown entity",color:"#ff66cc",delay:500},{text:"🧠 terminal.exe ready. Type /manual",color:"#ffcc00",delay:300}];H.textContent="",O.disabled=!0,b.disabled=!0;const i=document.getElementById("sfx-boot");i&&(i.currentTime=0,i.play());const c=document.createElement("span");H.appendChild(c);let I=0;function E(){if(I<d.length){const u=d[I];c.textContent=u.text,c.style.color=u.color,I++,setTimeout(E,u.delay)}else H.appendChild(document.createTextNode(`
`)),O.disabled=!1,b.disabled=!1,O.focus(),l()}E()})}const D_={neo:{background:"#000000",text:"#00ffcc",accent:"#ff66cc"},vapor:{background:"#1f0037",text:"#ff8aff",accent:"#8affff"},og:{background:"#111111",text:"#00ff00",accent:"#ff0000"},pink:{background:"#0a0a0a",text:"#ff3399",accent:"#ffcc00"}};function u_(l){const d=D_[l];d&&(document.body.style.background=d.background,document.body.style.color=d.text,document.querySelectorAll("input, select").forEach(i=>{i.style.color=d.accent,i.style.borderColor=d.text,i.style.background=d.background}),document.querySelectorAll("button").forEach(i=>{i.style.background=d.accent,i.style.color="black"}),localStorage.setItem("theme",l))}function U_(){const l=localStorage.getItem("theme");l&&u_(l)}function E_(){requestAnimationFrame(()=>{H.scrollTop=H.scrollHeight})}const n_=[{name:"The Fool",card:`
        .-------------------.
        |         0  *  *{  |
        |             *  *\`~|
        |       /P  *   *  *|
